...
def run_nextflow(exp_dir: str = "") -> subprocess.Popen:
    nextflow_command: str = (
        f"nextflow run {LABEL}.nf -c {INPUT_CONFIG} -with-timeline {OUT_LOG.rstrip('.log')}-timeline.html -with-trace {OUT_LOG.rstrip('.log')}-trace.txt -with-report {OUT_LOG.rstrip('.log')}-report.html".split()
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        WORKFLOW = f"/home/cc/case-studies/GenoX-Artifacts/experiments/nf_scripts/{args.app}.nf"
        INPUT_CONFIG = f"/home/cc/case-studies/GenoX-Artifacts/experiments/configs/{args.app}.config"
        PATH_CONTROLLER = (
