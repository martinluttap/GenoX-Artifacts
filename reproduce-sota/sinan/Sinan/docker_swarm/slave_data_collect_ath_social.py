...
def create_container_stats(service, container_name, container_id):
	...
	print("Create stats for container %s of %s" %(container_name, service))
...
def docker_ps():
	...
	for i in range(1, len(texts)):
		...
		print("docker ps container_name = %s, container_id = %s service = %s" %(c_name, c_id, service))
...
def get_replica():
	...
	print(f"services={Services}, containers={Containers}, Replica dict: {replica_dict}")
	return replica_dict
	...
	while True:
		...
		for container in ContainerList:
			...
			with open(str(pseudo_file), 'r') as f:
				...
				# print(container + ' docker cummulative cpu time: ' + \
...
def get_io_usage():
	...
	for container in ContainerList:	
		...
		# pseudo_file = '/sys/fs/cgroup/blkio/docker/' + str(ContainerIds[container])  + '/blkio.sectors'
		...
		# io services
		# pseudo_file = '/sys/fs/cgroup/blkio/docker/' + str(ContainerIds[container])  + '/blkio.io_serviced'
		pseudo_file = Path('/sys') / 'fs' / 'cgroup' / 'blkio' / 'docker' / ContainerStats[container]['id'] / 'blkio.throttle.io_serviced'
		with open(str(pseudo_file), 'r') as f:
			lines = f.readlines()
			for line in lines:
				if 'Total' in line:
...
def start_experiment(host_sock):
	...
	while True:
		...
		while '\n' in MsgBuffer:
			...
			print('cmd: ' + cmd)
			print(get_pod_map('social-network', Services))

			if 'get_info' in cmd:
				cur_time = time.time()
				print('time since last host query: ' + format(cur_time - prev_host_query_time, '.2f') + 's')
				# replica_dict = get_replica()
				docker_cpu_time = get_docker_cpu_usage()
				print(f'In get_info: docker_cpu_time={docker_cpu_time}')
...
def get_pod_map(namespace, components):
	print(f'get_pod_map: namespace={namespace}, components={components}')
	name_to_uid = {}
...
def main():
	...
	#------------------------------------
	print(f'{os.path.basename(__file__)}: Running server on {ServerPort}')
		...
		while '\n' in MsgBuffer:
			...
			if 'init_data' in cmd:
				...
				if not stat:	# experiment failed
					...
					break
				if len(MsgBuffer) > 0:
					print('Cmds left in MsgBuffer (after exp complete): ' + MsgBuffer)
					MsgBuffer = ''
...
if __name__ == '__main__':
	...
	print('SHOULD GET THIS ON SCREEN???')
	main()