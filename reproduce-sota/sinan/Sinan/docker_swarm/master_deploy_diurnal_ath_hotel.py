...
parser.add_argument('--viol-timeout', dest='viol_timeout', type=int, default=20)
...
DataDir =  Path('.') / 'logs' / 'hotel_diurnal_deploy_data'
...
def run_exp(diurnal_load, log_dir):
	...
	for load_spec in diurnal_load:
		...
		locust_p = run_locust_docker_compose(
			...
			duration=ExpTime+15, users=users, 
		...
		while time.time() - StartTime < exp_time + 15 - time_before_warmed:
			cur_time = time.time()
