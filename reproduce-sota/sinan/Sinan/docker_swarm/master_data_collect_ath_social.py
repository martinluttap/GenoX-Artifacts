...
from locust_util import (
	run_locust_docker_compose, run_locust, copy_locust_stats, clear_locust_state, _get_int_val, _get_float_val, get_locust_users, get_locust_data
)
from master_slave_msg import setup_server_slave, setup_slaves, connect_slave, send_server_terminate_exp, send_terminate_exp, send_terminate_slave, send_server_init_data, send_init_data, send_exp_start, send_server_rsc_config, send_rsc_config, _init_record, get_server_slave_metric, get_slave_metric, send_server_update_replica, send_update_replica
from docker_swarm_util import setup_swarm, update_node_label, docker_stack_rm, docker_stack_deploy, docker_check_replica, docker_service_scale, ReplicaState
	...
	def derive_per_replica(self, actual_replica):
		...
		for service in self.cpu:
			...
			per_replica_action.cpu[service] = ceil_float(num=self.cpu[service]/r,
				precision=4)
			print('derive_per_replica service %+20s total=%d, per_replica=%f' %(service, self.cpu[service], per_replica_action.cpu[service]))
...
def get_metric(feature):
	...
	print(f'Stats accum: {stats_accum}')
...
class HiddenState:
	...
	def propose_action(self):
		...
		if coin < ExplorationRate:
			...
			for service in ServiceConfig:
				...
				print('propose_action----explore %s, cpus: %d' %(service, cpu))
			return action
			...
			for service in ServiceConfig:
				...
				print('propose_action----hold %s, cpus: %d' %(service, ServiceConfig[service]['cpus']))
			return action
		...
		for service in ServiceConfig:
			...
			print('propose_action----op:%s, %+20s, cpus: %.2f, reward: %.8f, cpu_util: %.4f' %(chosen_op,
	...
	def propose_recover_action(self):
		...
		for service in ServiceConfig:
			...
			action.cpu[service] = chosen_cpu
			action.core[service] = math.ceil(chosen_cpu)
			print('propose_action----op:%s, %s, cpus: %d, reward: %.8f' %(chosen_op,
...
def do_service_docker_scale(stack_name, service, next_r, replica_state):
	print(f'Trying to scale {stack_name} service={service},next_r={next_r}, replica_state={replica_state}')
...
def do_docker_scale_init():
	...
	time.sleep(5)
	print(f'do_docker_scale_init: {ServiceInitConfig}')
	for service in ServiceInitConfig:
		if service == 'jaeger' or service == 'zipkin':
			continue
		actual, desired = docker_check_replica(stack_name=Stackname,
			service=service)
		print(f'service: {service}, actual: {actual}, desired: {desired}')
...
def inform_slaves_new_replica():
	...
	for s in ServiceReplicaStates:
		if ServiceReplicaStates[s].slave_need_informed():
			print("service %s needs informed of replica update" %s)
...
def run_exp(users, log_dir, nginx_ip):
	global ViolTimeout
	...
	locust_p = run_locust(
		...
		nginx_ip=nginx_ip, volumes=LocustVolumes,
		log_file=LocustLog, duration=ExpTime + 120,
		users=users, quiet=True)

	print(f'Users {users}: locust_p={locust_p}')
	assert(locust_p != None)

	time.sleep(10)	# wait for latencys to become stable
	...
	while time.time() - StartTime < ExpTime:
		...
		if cur_time - StartTime < MeasureInterval*interval_idx:
			...
			print('current interval_idx: ' + str(interval_idx) + ', cur_time = ' + format(cur_time - StartTime, '.1f'))
			interval_idx = int((cur_time - StartTime)/MeasureInterval) + 1
			print('Before inform slaves new replica')
			inform_slaves_new_replica()
			print('After inform slaves new replica')
			# prev_time  = cur_time
			...
			print(f'tail={tail}, rps={rps}, failures={failures}')
			success = get_metric(feature=feature)
			print(f'Get metric success: {success}')
			if not success:
				exp_terminated = True
				service_fail = True
				break
...
def get_nginx_address(servers) -> str:
	pod_name = os.popen('kubectl get pods --all-namespaces -o wide | grep nginx | tr -s \' \' | cut -d \' \' -f 2').read().strip()
	node_name = os.popen('kubectl get pods --all-namespaces -o wide | grep nginx | tr -s \' \' | cut -d \' \' -f 8').read().strip()
	
	service_name = '-'.join(pod_name.split('-')[:-2])
	port = os.popen(f'kubectl get services --all-namespaces -o wide | grep {service_name} | tr -s \' \' | cut -d \' \' -f 6').read().strip().split(':')[1].rstrip('/TCP')

	return f"http://{servers[node_name]['ip_addr']}:{port}"

def run_benchmark_init(benchmark, benchmark_dir, servers):
	print(f'Running benchmark init: benchmark={benchmark}, benchmark_dir={benchmark_dir}, servers={servers}')
	assert 'social' in Benchmark
	if 'social' in Benchmark:
		nginx_ip = get_nginx_address(servers)
		print(f'Running setup_social_graph_init_data_sync.py')
		# cmd = 'python3 ' + str(BenchmarkDir / 'scripts' / 'setup_social_graph_init_data_sync.py') + \
		# 	' ' + str(BenchmarkDir / 'datasets' / 'social-graph' / 'socfb-Reed98' / 'socfb-Reed98.mtx')

		cmd = 'python3 ' + str(BenchmarkDir / 'scripts' / 'autothrottle_setup_social_graph_init_data_sync.py') + ' ' + str(nginx_ip)

		sp.call(cmd, shell=True, stdout=sys.stdout, stderr=sys.stderr, preexec_fn=os.setsid,bufsize=-1)
		# print 'setup_social_graph_init_data.py out: ', out
		print('social network set up done')
		time.sleep(5)
...
def main():
	...
	print(f'TestUsers: {TestUsers}') 
	print(f'DataDir: {DataDir}') 
	print(f'Services: {Services}') 
	print(f'SetupSwarm: {SetupSwarm}') 
	print(f'Deploy: {Deploy}') 

	print(f'TotalSamples: {TotalSamples}') 
	print(f'ViolSamples: {ViolSamples}') 
	print(f'SatSamples: {SatSamples}') 
	print(f'NonRoiSamples: {NonRoiSamples}') 

	print(f'Servers: {Servers}') 
	print(f'HostServer: {HostServer}') 
	print(f'SlavePort: {SlavePort}') 
	print(f'SlaveSocks: {SlaveSocks}') 

	print(f'BenchmarkDir: {BenchmarkDir}') 
	...
	if SetupSwarm:
		...
		print(f'Setting up swarm ... Worker nodes:{worker_nodes}')
	...
	print(f'master_data_collect: {slaves}')
	time.sleep(1)
	...
	print(f'connect_slave done.')
	
	# data collection
	i = 0

	# run_benchmark_init(benchmark=Benchmark, benchmark_dir=BenchmarkDir, servers=Servers)

	while i < len(TestUsers):
		print(f'[{i}/{len(TestUsers)}]')
		users = TestUsers[i]
		# if Deploy:
		# 	print(f'Deploy ...')
			# converged = False
		...
		print(f'Done deploying')
		users_dir = DataDir / ('users_' + str(users))
		print(f'Users dir: {users_dir}')
		service_fail = run_exp(
			users=users, 
			log_dir=users_dir,
			nginx_ip=get_nginx_address(Servers)
		)
		print(f'Service fail: {service_fail}')
		if not service_fail:
			i += 1
		time.sleep(10)
	...
	print('total samples: %d, sat: %d, viol: %d, non_roi: %d' %(TotalSamples, 
