...
def setup_gpu(username, gpu_host, work_dir, script, gpus):
	slave_cmd = 'source ~/.bashrc ; pyenv activate sinan ; ' + 'cd ' + work_dir + ';'
	if len(gpus) > 0:
		slave_cmd += ' /users/martinlp/.pyenv/shims/python3 ' + script + ' --gpus ' + ','.join([str(g) for g in gpus])
	else:
		slave_cmd += ' /users/martinlp/.pyenv/shims/python3 ' + script
