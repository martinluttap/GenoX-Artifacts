...
def setup_server_slave(stack_name, username, server, server_cpus, slave_port, 
	...
	# 	identity_file='')
	# exp_script = f"{slave_script_dir}/slave_data_collect_ath_social.py"
	# scp(source=str(exp_script), target=username+'@'+server+':'+ str(exp_script),
	# 	identity_file='')
	...
	slave_cmd += 'python3 ' + str(slave_script_dir / 'slave_data_collect_ath_social.py') + \
				...
				' 2>&1 | tee $(date \'+%d-%m-%Y_%H-%M-%S\')-slave_social.txt'
	# print(slave_cmd)
	# print(f'setup_server_slave: {username}@{server}:{slave_port}')
...
def setup_slaves(stack_name, username, servers, 
	...
	for server in servers:
		print(f'Setup slaves {username}@{server}:{slave_port}, script_dir:{slave_script_dir}, config_path={config_path}, quiet={quiet}')
		# ip_addr = servers[server]['ip_addr']
		server_cpus = servers[server]['cpus']
		
		# setup_server_slave: copy config.json and run slave_data_collect_ath_social.py
	...
	print(f'Setup slaves: {p_list}')
	return p_list

def connect_slave(servers, slave_port, slave_socks):
	print(f'In connect_slave: servers={servers}, port={slave_port}, socks={slave_socks}')
	for server in servers:
...
def send_server_terminate_exp(server, sock):
	...
	while True:
		...
		while '\n' in msg:
			(cmd, rest) = msg.split('\n', 1)
			msg = rest
			print('recv %s from %s' %(cmd, server))
...
def send_terminate_exp(servers, slave_socks):
	...
	for t in t_list:
		t.join()
	print('experiment fully terminated')
...
def get_server_slave_metric(server, record, sock):
	...
	msg = ''
	print(f'Server: {server}, record={record}, sock={sock}')
	while True:
		...
		if '\n' not in msg:
			...
			# debug
			print('recv metric from %s' %server)
...
def send_update_replica(servers, slave_socks, service_list):
	...
	for t in t_list:
		t.join()
	print('send_update_replica done')